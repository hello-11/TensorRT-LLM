...
// template <>
// void invokeUnfusedGenMLASoftmax<__nv_bfloat16>(__nv_bfloat16 const* input, __nv_bfloat16* output,
//     int const* seqLensKvPtr, int q_seq_len, int max_kv_seq_len, float softmax_scale, int max_q_seq_len, int
//     num_heads, cudaStream_t stream)
// {
//     dim3 grid(q_seq_len);
//     dim3 block(256);

//     basic_unfused_gen_mla_softmax_kernel<__nv_bfloat16><<<grid, block, 0, stream>>>(
//         input, output, q_seq_len, max_kv_seq_len, softmax_scale, max_q_seq_len, num_heads, seqLensKvPtr);
// }
#endif
